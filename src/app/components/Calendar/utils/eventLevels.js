...
export function eventSegments(segment, events, first, last, { startAccessor, endAccessor }, range) {
    let slots = dates.diff(first, last, 'day');
    let start = dates.max(dates.startOf(get(segment, startAccessor), 'day'), first);
    let end = dates.min(dates.ceil(get(segment, endAccessor), 'day'), last);
